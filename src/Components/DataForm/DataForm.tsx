...
const CollectDataForm: React.FC<SimpleDialogProps> = (prop) => {
    ...
    const [leaveType, setLeaveType] = useState<'none' | 'full' | 'half'|'holiday'>('none');
    ...
    const handleLeaveTypeChange = (type: 'full' | 'half'|'holiday') => {
        setLeaveType(type);
        if (type === 'full') {
            setForm(prev => ({ ...(prev || {}), leaves: '1', status: 'Leave', loginTime: '', logoutTime: '', project: '', task: '' }));
        } 
            ...
            setForm(prev =>({...(prev || {}), leaves:'0',status:'Enjoy Your Holiday' }))
        }
    };
    ...
    const textfieldStyle = {'& .MuiOutlinedInput-input': { color: 'white',
    '&:-webkit-autofill': { WebkitTextFillColor: "rgb(255,255,255)", }, }}
                ...
                <Dialog open={prop.open} onClose={() => prop.onClose()} >
                    ...
                    <DialogContent sx={dialogContentStyle}>
                        <Stack direction={"row"} gap={3} sx={{ mt: 2 }}>
                            <TextField required variant="outlined" label="Name" name="name" value={form.name} onChange={handleChange} sx={textfieldStyle}></TextField>
                        ...
                        {onLeave && (
                            <Stack direction={"row"} gap={2} sx={{ mt: 1, alignItems: 'center' }}>
                                ...
                                />
                                <FormControlLabel
                                    control={<Checkbox checked={leaveType === 'holiday'} onChange={() => handleLeaveTypeChange('holiday')} />}
                                    label="Holiday"
                                />
                            </Stack>
                        )}
